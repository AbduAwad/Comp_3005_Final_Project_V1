...
def Q_1(cursor, conn, execution_time):
    ...
    cursor.execute("""
                    ...
                    JOIN eventDetails ON events.event_id = eventDetails.event_id 
                    WHERE event_name = 'Shot' AND event_attribute = 'statsbomb_xg' 
                    AND match_id IN (SELECT match_id FROM matches WHERE competition_id = 11 
                    AND season_id = 90) GROUP BY player_name ORDER BY AVG(event_value::float) DESC;
                    """)
...
def Q_2(cursor, conn, execution_time):
    ...
    #==========================================================================    
    # Enter query and create .csv here...
    cursor.execute("""SELECT player_name FROM events 
                   WHERE event_name = 'Shot' 
                   AND match_id IN (SELECT match_id FROM matches 
...
def Q_3(cursor, conn, execution_time):
    ...
    cursor.execute("""
                    SELECT player_name FROM events 
                   JOIN eventDetails ON events.event_id = eventDetails.event_id 
                   WHERE event_name = 'Shot' AND event_attribute = 'first_time' 
                   AND match_id IN (SELECT match_id FROM matches 
                   WHERE competition_id = 11 AND season_id IN (90, 79, 72)) GROUP BY player_name 
                   ORDER BY COUNT(event_value) DESC;""")
...
def Q_4(cursor, conn, execution_time):
    ...
    # Enter query and create .csv here...
    cursor.execute("""                
SELECT team_name FROM events JOIN eventDetails ON events.event_id = eventDetails.event_id WHERE event_name = 'Pass' AND event_attribute = 'recipient' AND match_id IN (SELECT match_id FROM matches WHERE competition_id = 11 AND season_id = 90) GROUP BY team_name ORDER BY COUNT(event_value) DESC;""")
...
def Q_5(cursor, conn, execution_time):
    ...
    # Enter query and create .csv here...
    cursor.execute("""
                    SELECT player_name FROM events JOIN eventDetails ON events.event_id = eventDetails.event_id WHERE event_name = 'Pass' AND event_attribute = 'recipient' AND match_id IN (SELECT match_id FROM matches WHERE competition_id = 2 AND season_id = 44) GROUP BY player_name ORDER BY COUNT(event_value) DESC;""")
...
def Q_6(cursor, conn, execution_time):
    ...
    # Enter query and create .csv here...
    cursor.execute("""
SELECT team_name FROM events JOIN eventDetails ON events.event_id = eventDetails.event_id WHERE event_name = 'Shot' AND event_attribute = 'outcome' AND event_value = 'Goal' AND match_id IN (SELECT match_id FROM matches WHERE competition_id = 2 AND season_id = 44) GROUP BY team_name ORDER BY COUNT(event_value) DESC;                    
""")
...
def Q_7(cursor, conn, execution_time):
    ...
    # Enter query and create .csv here...
    cursor.execute("""
                    SELECT player_name FROM events JOIN eventDetails ON events.event_id = eventDetails.event_id WHERE event_name = 'Pass' AND event_attribute = 'through_ball' AND match_id IN (SELECT match_id FROM matches WHERE competition_id = 11 AND season_id = 90) GROUP BY player_name ORDER BY COUNT(event_value) DESC;""")
...
def Q_8(cursor, conn, execution_time):
    ...
    # Enter query and create .csv here...
    cursor.execute("""
SELECT team_name FROM events JOIN eventDetails ON events.event_id = eventDetails.event_id WHERE event_name = 'Pass' AND event_attribute = 'through_ball' AND match_id IN (SELECT match_id FROM matches WHERE competition_id = 11 AND season_id = 90) GROUP BY team_name ORDER BY COUNT(event_value) DESC;
                    """)
...
def Q_9(cursor, conn, execution_time):
    ...
    cursor.execute("""
            ...
            FROM events
            JOIN eventDetails ON events.event_id = eventDetails.event_id
            WHERE event_name = 'Dribble'
                AND event_attribute = 'outcome'
                AND event_value = 'Complete'
                AND match_id IN (
                    SELECT match_id
                    FROM matches
                    WHERE competition_id = 11
                        AND season_id IN (90, 42, 4)
                )
            GROUP BY player_name
            ORDER BY COUNT(event_value) DESC;
                   """)
...
def Q_10(cursor, conn, execution_time):
    ...
    cursor.execute("""
        SELECT player_name
                    FROM events
                    WHERE event_name = 'Dribbled Past'
                        AND match_id IN (
                            SELECT match_id
                            FROM matches
                            WHERE competition_id = 11
